...
The following keys are recognized:
  ...
  (let* ((args (elpaca-test--args body))
         ...
         (localp (eq ref 'local))
         (init-file (cond ((eq (car (car-safe init)) :file)
         ...
         (early-file (cond ((eq (car (car-safe early)) :file) (eval (cadar early) t))
    ...
    `(let* ((default-directory ,dir)
       ...
       (elpaca--test-write-init ,init-file ',ref ',(when (or localp (null init-file)) init))
