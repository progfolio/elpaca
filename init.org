;-*- eval: (progn (auto-tangle-mode) (eldoc-mode) (add-hook 'auto-tangle-before-tangle-hook #'check-parens) (add-hook 'auto-tangle-after-tangle-hook (lambda () (load-file "~/.emacs.d/init.el"))));   -*-
#  LocalWords:  TODOS yasnippet magit epub LocalWords MELPA ELPA init Spacemacs symlinked Elisp gc PDF Paren alist recentf smtpmail modeline zerodark quickhelp esup TLS vterm flycheck elfeed flyspell autoload descbinds :PROPERTIES: evilified
...
#+startup: indent
* An Explanation
This is a literate configuration for =Emacs=. Tangling this file creates an Elisp
file, =~/.emacs.d/lisp/bootstrap.el=, which is then byte-compiled and loaded in =~/emacs.d/init.el=.
* Bootstrapping
** early-init
:PROPERTIES:
...
Emacs native branch can asynchronously compile packages to .eln files after they are lazy-loaded.
...
*** Debugging
Running this form will hit the debugger after loading a package.
Useful for finding out what dependency is requiring a package (perhaps earlier than you want).
Usually snags me when running a different version of a package that Emacs includes by default.
...
*** gc-cons threshold
Emacs collects garbage every 800KB. This is overly aggressive
on a modern machine during our init. Temporarily turning it off should
decrease startup times. Resetting it afterward will ensure that normal
operations don't suffer from a large GC period.
...
Keychain is allows me to cache my ssh credentials.
...
** profiling
We want to measure how long Emacs takes to start. That way we can have
a general sense of when to profile or try, if at all, to optimize the
init file.

This function displays how long Emacs took to start. It's a rough way
of knowing when/if I need to optimize my init file.
#+begin_src emacs-lisp
...
** packaging
*** MELPA & use-package
MELPA is a third party package repository for Emacs. It has more
packages available than GNU ELPA (the default repository).

Since I'm using straight.el to manage packages, we want to avoid using package.el.
#+begin_src emacs-lisp :tangle no
(require 'package)
...
The following ensures packages are automatically installed and
bootstraps a package called =use-package=. Use-package allows us to
configure packages in a clean/compartmentalized fashion. It bundles the package
installation code and configuration together.
...
#+end_src
* Secrets
I keep my sensitive, personal information in a separate file so I can publish
this configuration publicly.
#+begin_src emacs-lisp
...
#+end_src
** auto-fill-mode
I usually want prose to split at 80 chars (Emacs defaults to 70). I'd rather have
to toggle it off than on.
#+begin_src emacs-lisp
(use-feature simple
  :general
...
#+end_src
** auto-revert
Automatically revert a buffer if it's associated file has changed on
disk. This is what you would expect to happen when checking out
different versions of a file in version control. It also helps if
multiple instances of Emacs are open so that you don't unwittingly
clobber the same file.

I'm setting the =auto-revert-interval= low enough that it effectively
reverts immediately. This seems preferable to me for now. Not sure if
it will cause problems with longer running processes.
#+begin_src emacs-lisp
...
(use-package company
  ...
  (global-company-mode))
;; (setq-default company-backends '((company-files company-keywords company-capf company-yasnippet)
;;                                  (company-abbrev company-dabbrev))
;;               company-frontends '(company-tng-frontend
;;                                   company-pseudo-tooltip-unless-just-one-frontend company-echo-metadata-frontend
;;                                   company-preview-if-just-one-frontend))
;; ;;for company-ispell backend
;; ;;(setq ispell-alternate-dictionary "/usr/share/dict/words")
;; ;;(add-hook 'text-mode-hook (lambda () (set (make-local-variable 'company-backends)
...
#+end_src
** elfeed
I've put my elfeed database in under version control. At some point I may move
it from it's default location (=~/.elfeed=), stored in the =elfeed-db-directory= variable.
#+begin_src emacs-lisp
(use-package elfeed
  :commands (elfeed)
  :config
...
#+end_src
** Emacs
Defined in C code, so we use the ~emacs~ pseudo-package to set them.
#+begin_src emacs-lisp
(use-feature emacs
  :demand t
  :custom
...
#+end_src
** files
By default Emacs saves backups in the current buffer's working
directory. I'd rather have everything in one folder to keep my file system tidy.
#+begin_src emacs-lisp
(use-feature files
  :hook
...
#+end_src
** holidays
I'd like to see holidays and anniversaries in my org-agenda and calendar, but I don't need
some of the holiday lists that come by default.
#+begin_src emacs-lisp
...
#+end_src
** novice
Novice tries to help new users by disabling certain potentially
...
Align tags to 80th column in the agenda view:
#+begin_src emacs-lisp
...
#+end_src
** yasnippet
YASnippet expands text based off of templates. Handy for automating things like
...
Has something to do with the mini-buffer according to this:
...
Not sure why that hook is appropriate, but calling 'evil-ex-search-exit'
manually solves the issue as well. Will look into it more later.
** ace-window
#+begin_src emacs-lisp
...
(use-package outshine
  :general
  ;;@TODO: fix this later. It shouldnt be in "narrow" menu.
  ;;but we need to make an evil-intercept map similar to what is done
  ;;in company config.
  (+general-global-narrow
    "o" '(:ignore t :which-key "outshine")
    "oc" 'outshine-cycle
...
(use-feature hideshow
  :general
  (+general-global-narrow
    ;; for now we're just using the toggle, may add more later.
...
(use-package fuzzy-finder
  :straight (:type git :host github :repo "10sr/fuzzy-finder-el" :fork "progfolio/fuzzy-finder-el"))
#+end_src
** cleanroom
#+begin_src emacs-lisp
