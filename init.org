...
(defvar +git-server-address "192.168.1.180")
...
NAME and ARGS are in `use-package'."
  (declare (indent defun))
  `(use-package ,name
     :straight (,name :repo ,(concat "git@" +git-server-address ":emacs/"
...
(use-feature dired
  :defer t
  :custom (dired-listing-switches "-alh" "Human friendly file sizes.")
  :general
...
(use-private org-live-preview
  :disabled t ;requires an outdated 'utils' function
  :after (org)
...
(use-package org-roam
  :straight (:type git :host github :repo "org-roam/org-roam" :fork t :branch "v2")
  :general
  (+general-global-application
