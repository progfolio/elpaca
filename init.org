...
(when after-init-time
  (setq elpaca--queues (cl-remove 'init elpaca--queues :key #'elpaca-q<-type))
  (elpaca-split-queue))
...
(use-package ement
  ...
  (add-hook 'epa-key-list-mode-hook #'exit-recursive-edit)
  :general
  (+general-global-application
    "e" '(:ignore t :which-key "Matrix")
    "ee" '(lambda () (interactive) (call-interactively (if (bound-and-true-p ement-sessions) #'ement-room-list #'ement-connect)))
    "el" 'ement-room-list
    "en" 'ement-notify-switch-to-notifications-buffer
    "em" 'ement-notify-switch-to-mentions-buffer))
#+end_src
** emp
#+begin_quote
MPV integration
...
#+begin_src emacs-lisp :lexical t
:config
;; speed up org-agenda-todo
(define-advice org-agenda-todo (:around (fn &rest args) no-org-indent-refresh)
  (cl-letf (((symbol-function 'org-indent-refresh-maybe) #'ignore))
    (funcall fn args)))
