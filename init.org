...
(add-hook 'emacs-startup-hook (lambda () (unless +parcel-installed (parcel-display-status-buffer))))
#+end_src

Bury the status buffer when everything initialized OK.
#+begin_src emacs-lisp :lexical t
(add-hook 'parcel-after-init-hook
          (lambda () (when-let ((buffer (get-buffer parcel-status-buffer)))
...
(parcel-split-queue)
(parcel '(parcel-menu-demo :host github :repo "progfolio/parcel-menu-demo" :protocol ssh)
  (add-to-list 'parcel-menu-functions 'parcel-menu-demo))
(parcel-split-queue)
#+end_src
  ...
  (defun +flymake-toggle-diagnostics-buffer ()
    ...
    (if flymake-mode
      ...
      (when-let ((root (vc-root-dir))
                 (window (get-buffer-window (flymake--project-diagnostics-buffer root))))
