...
(with-eval-after-load 'parcel
  (add-hook 'parcel-after-init-hook '+reset-init-values))
#+end_src
...
#+begin_src emacs-lisp :lexical t
(add-hook 'parcel-after-init-hook
          (lambda ()
...
(add-hook 'parcel-after-init-hook (lambda () (profiler-stop) (profiler-report)))
#+end_src
ELP is useful for seeing which functions in a package are "hot".
#+begin_src emacs-lisp :lexical t :tangle no
(require 'elp)
(with-eval-after-load 'parcel
  (elp-instrument-package "parcel"))
(add-hook 'parcel-after-init-hook
          (lambda () (elp-results) (elp-restore-package 'parcel)))
#+end_src
...
https://github.com/progfolio/parcel
#+end_quote
...
I want to see the status buffer when doing an initial install.
#+begin_src emacs-lisp :lexical t
(setq +parcel-installed (file-exists-p (expand-file-name "repos" parcel-directory)))
(add-hook 'emacs-startup-hook (lambda () (unless +parcel-installed (parcel-status))))
#+end_src

Bury the status buffer when everything initialized OK.
#+begin_src emacs-lisp :lexical t
(add-hook 'parcel-after-init-hook
          (lambda () (when-let (((featurep 'parcel-status))
                                (buffer (get-buffer parcel-status-buffer)))
...
(with-eval-after-load 'evil
  (with-eval-after-load 'parcel-status
    (evil-make-intercept-map parcel-status-mode-map))
  (with-eval-after-load 'parcel-ui
    (evil-make-intercept-map parcel-ui-mode-map)))
#+end_src

[[https://www.funtoo.org/Keychain][Keychain]] is a gpg/ssh agent that allows me to cache my credentials.
This package gets the correct environment variables so parcel can use the ssh protocol.

#+begin_src emacs-lisp :lexical t
;; We need this loaded for SSH protocol
(parcel-queue
 (parcel keychain-environment
   (keychain-refresh-environment)))
(parcel-queue
 (parcel (melpulls :host github :repo "progfolio/melpulls" :protocol ssh)
   (add-to-list 'parcel-menu-functions 'melpulls)))
#+end_src
...
#+begin_src emacs-lisp :lexical t
(parcel use-package (require 'use-package))
#+end_src
...
There are a couple of keybindings that should be in evil's parcel-use-package form that are in general's until I solve the problem.
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package evil-collection
  :after (evil)
...
** accord
#+begin_src emacs-lisp :lexical t
(parcel-use-package (accord :host github :repo "progfolio/accord" :protocol ssh)
  :general
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package afternoon-theme :defer t)
#+end_src
** almost-mono-themes
#+begin_quote
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package almost-mono-themes :defer t)
#+end_src
** asm-mode
#+begin_src emacs-lisp :lexical t
...
** auto-tangle-mode
#+begin_src emacs-lisp :lexical t
(parcel-use-package (auto-tangle-mode
                     :host github
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package buttercup
  :commands (buttercup-run-at-point))
#+end_src
** cleanroom
#+begin_src emacs-lisp :lexical t
(parcel-use-package (cleanroom :host github :repo "progfolio/cleanroom" :protocol ssh)
  :defer t)
#+end_src
** compile
#+begin_src emacs-lisp :lexical t
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (corfu :host github :repo "minad/corfu" :files (:defaults "extensions"))
  :defer 5
  :custom
  (corfu-cycle t)
  :config
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package default-text-scale
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (doct :branch "development" :protocol ssh :depth nil)
  :commands (doct))
#+end_src
** doom-modeline
#+begin_quote
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package doom-modeline
  :config
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package doom-themes :defer t)
#+end_src
** edebug
#+begin_quote
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (emp :host github :repo "progfolio/emp")
  :config
  :general
  (+general-global-application
...
** epoch
#+begin_src emacs-lisp :lexical t
(parcel-use-package (epoch :host github :repo "progfolio/epoch")
  :after (org)
...
#+begin_src emacs-lisp :lexical t
(parcel-use-package flycheck-package
  :after (flychceck)
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package flyspell-correct
  :after (flyspell)
  :general
  (+general-global-spelling
...
#+begin_src emacs-lisp :lexical t
(parcel-use-package fontify-face
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package fountain-mode
  :mode "\\.fountain\\'")
#+end_src
** helm
#+begin_quote
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package helm-describe-modes
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package helm-descbinds
  :after (helm)
...
*** helm-projectile
#+begin_src emacs-lisp :lexical t
(parcel-use-package helm-projectile
  :after (helm)
  :general
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package highlight-indent-guides
  :commands
...
#+begin_src emacs-lisp :lexical t
(parcel-use-package (i3wm-config-mode
...
#+begin_src emacs-lisp :lexical t
(parcel-use-package (lilypond-mode :host gitlab :repo "lilypond/lilypond"
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package markdown-mode
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (mu4e :host github :files ("build/mu4e/*.el") :repo "djcb/mu"
                          :pre-build (("./autogen.sh") ("ninja" "-C" "build")))
  :commands (mu4e mu4e-update-index)
  :custom
  (mail-user-agent 'mu4e-user-agent)
  (mu4e-mu-binary (expand-file-name "repos/mu.djcb.github/build/mu/mu" parcel-directory))
  (mu4e-org-support t)
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (nov :depth nil)
  :custom
  (nov-text-width 80)
  :mode
  ("\\.epub\\'" . nov-mode)
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package olivetti
  :commands (olivetti-mode))
#+end_src
** org
#+begin_src emacs-lisp :lexical t
(parcel-use-package org
  :defer t
  :general
...
** org-better-repeat
#+begin_src emacs-lisp :lexical t
(parcel-use-package (org-better-repeat-mode :host github :repo "progfolio/org-better-repeat-mode" :protocol ssh)
  :after (org)
  :config
...
#+begin_src emacs-lisp :lexical t
(parcel-use-package org-bullets
  :after (org)
...
** org-chronicle
#+begin_src emacs-lisp :lexical t :tangle no
(parcel-use-package org-chronicle
...
** org-clean-refile
#+begin_src emacs-lisp :lexical t
(parcel-use-package (org-clean-refile :host github :repo "progfolio/org-clean-refile" :protocol ssh)
...
** org-contrib
#+begin_src emacs-lisp :lexical t
(parcel-use-package org-contrib)
#+end_src
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package org-fancy-priorities
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package org-make-toc
  :commands (org-make-toc))
#+end_src
** org-region-link
#+begin_src emacs-lisp :lexical t
(parcel-use-package (org-region-link :host github :repo "progfolio/org-region-link" :protocol ssh)
  :after (org))
#+end_src
** org-roam
#+begin_src emacs-lisp :lexical t
(parcel-use-package (org-roam :host github :repo "org-roam/org-roam")
  :disabled t
  :general
  (+general-global-application
...
** org-relativity
#+begin_src emacs-lisp :lexical t :tangle no
(parcel-use-package org-relativity
  :after (org)
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (org-superstar :host github :repo "integral-dw/org-superstar-mode")
  :after (org))
#+end_src
** ox-twbs
#+begin_quote
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package package-lint
  :defer t
  :commands (package-lint-current-buffer +package-lint-parcel)
  :config
  <<+package-lint-parcel>>)
#+end_src
*** +package-lint-parcel
:PROPERTIES:
:header-args: :noweb-ref +package-lint-parcel
:END:
package-lint assumes package.el is the package manager.
I use parcel.el, so I get spurious warnings about uninstallable packages.
This workaround creates a temporary package archive and enables package.el to appease package-lint.

#+begin_src emacs-lisp :lexical t
(defun +package-lint-parcel ()
  "Help package-lint deal with parcel."
  (interactive)
  (require 'package)
...
(parcel-use-package projectile
  ...
  (projectile-mode))
#+end_src
** TODO pdf-tools (:post-build should be simpler. Are we processing :files correctly?)
#+begin_quote
...
#+begin_src emacs-lisp :lexical t
(parcel-use-package (pdf-tools :post-build ((condition-case err
                                                (let* ((repo-dir default-directory)
                                                       (build-dir (expand-file-name "builds/pdf-tools" parcel-directory))
                                                       (default-directory build-dir))
                                                  (require 'pdf-tools)
                                                  (setq pdf-tools-enabled-modes nil)
                                                  (pdf-tools-install 'noquery 'skip-deps)
                                                  (with-current-buffer "*compilation*"
                                                    (while (get-buffer-process (current-buffer))
                                                      ;; poll for async compilation output
                                                      (sit-for 0.25)
                                                      (when (buffer-modified-p)
                                                        (message (substring-no-properties (buffer-string))))))
                                                  (copy-file (expand-file-name "./lisp/epdfinfo" repo-dir) (file-name-as-directory build-dir)))
                                              ((error) (message "PDF-TOOLS ERROR: %S" err)))))
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package rainbow-mode
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (shackle :depth nil)
...
** skip-buffers
#+begin_src emacs-lisp :lexical t
(parcel-use-package (skip-buffers :host github :repo "progfolio/skip-buffers" :protocol ssh)
  :defer 10
  :config
...
** stardict-utils
#+begin_src emacs-lisp :lexical t
(parcel-use-package (stardict-utils :host github :repo "progfolio/stardict-utils" :protocol ssh)
  :after (general helm)
...
** straight.el
#+begin_src emacs-lisp :lexical t
(parcel (straight.el :host github
                     :depth nil
...
** joymacs
#+begin_src emacs-lisp :lexical t
(parcel-use-package (joymacs :host github :repo "skeeto/joymacs"
                             :pre-build ("make")
...
** padscape
#+begin_src emacs-lisp :lexical t
(parcel-use-package (padscape :host github :repo "progfolio/padscape" :protocol ssh)
  :commands (padscape))
#+end_src
** speedo
#+begin_src emacs-lisp :lexical t
(parcel-use-package (speedo :host github :repo "progfolio/speedo"
                            :local-repo "speedo"
                            :depth nil :protocol ssh
                            :files (:defaults "test"))
  :commands (speedo speedo-load-file akogare)
  :custom
  (speedo-directory "~/.emacs.d/parcel/repos/speedo.progfolio.github/test/")
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package (vterm :post-build
                           (progn
                             (setq vterm-always-compile-module t)
                             (require 'vterm)
                             ;;print compilation info for parcel
...
** wikinforg
#+begin_src emacs-lisp :lexical t
(parcel-use-package (wikinfo :host github :repo "progfolio/wikinfo"
                             :branch "develop" :protocol ssh :depth nil)
  :defer t)
#+end_src
#+begin_src emacs-lisp :lexical t
(parcel-use-package (wikinforg :host github :repo "progfolio/wikinforg"
...
** wordel
#+begin_src emacs-lisp :lexical t
(parcel-use-package (wordel :host github :repo "progfolio/wordel"
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package writegood-mode
...
#+end_quote
#+begin_src emacs-lisp :lexical t
(parcel-use-package zerodark-theme :defer t)
#+end_src
** yodel
#+begin_src emacs-lisp :lexical t
(parcel-use-package (yodel :host github :repo "progfolio/yodel" :depth nil :protocol ssh)
  :defer t)
#+end_src
