...
If SEPARATE is non-nil, launch a separate instance of mpv."
    (interactive "P")
    (mapc #'emp-open-url (nreverse (mapcar #'elfeed-entry-link (elfeed-search-selected))))
  ...
  (defun +elfeed-download ()
    ...
    (let ((default-directory (expand-file-name "~/Videos/youtube")))
      (dolist (link (nreverse (mapcar #'elfeed-entry-link (elfeed-search-selected))))
        (message "Attempting to download %S" link)
        (make-process
         :name "elfeed-download"
         :buffer "elfeed-download"
         :command (list "youtube-dl" link)
         :sentinel (lambda (process _event)
                     (when (= 0 (process-exit-status process))
                       (message "Successfully downloaded %S" link))))))
    (elfeed-search-untag-all-unread))
  :general
