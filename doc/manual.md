...
The `elpaca-write-lock-file` command writes a lock file to a specified path. The `elpaca-lock-versions` command is a convenience wrapper that writes to the path specified by `elpaca-lock-file`.
