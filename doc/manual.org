...
**** Package types
:PROPERTIES:
:CUSTOM_ID: package types
:END:
The =:type= recipe keyword allows specifying a symbol representing the type of package to install.
Elpaca searches for a matching =elpaca-types= entry to load a library supporting the type if necessary.
...
:PROPERTIES:
:CUSTOM_ID: general recipe keywords
:END:
...
:END:
A list of build steps executed in order.
Accepted values are:
- nil to disable all build steps
#+begin_src emacs-lisp :lexical t
(example :build nil) ;; Disable all build steps
#+end_src
- A substitution rule of the form =(TYPE TARGET SUBSTITUTIONS...)=
  The =SUBSTITUTIONS= are function symbols which replace the =TARGET= symbol.
...
- A list of substitution rules. Each rule is run in declared order and applied to the previous rule's results.
#+begin_src emacs-lisp :lexical t
...
2. Byte-compiling the package's elisp and generating package autoloads.
...
:END:
The files linked from the package's source directory to its build directory.
...
:PROPERTIES:
:CUSTOM_ID: repository recipe keywords
:END:
...
:END:
A symbol or string representing a repository's hosting service.
