...
(defun elpaca-log--entries ()
  ...
  (cl-loop
   ...
   (cl-loop
    ...
    for busyp = (eq type 'package-busy)
    for face = (plist-get payload :face)
    do (when-let* ((s (plist-get payload :status))) (setq current-status s))
    for entry =
...
;;; Log mode subscribes to events for real-time updates
...
(defun elpaca-log--subscribe ()
  ...
  (elpaca-subscribe-many
   `((package-info      . ,#'elpaca-log--on-event)
     (package-busy      . ,#'elpaca-log--on-event)
     (elpaca-status-changed . ,#'elpaca-log--on-event))))
