...
(defun elpaca--finalize-queue (q)
  ...
  (let ((next (nth (1+ (elpaca-q<-id q)) (reverse elpaca--queues))))
    ...
    (run-hook-with-args 'elpaca-after-queue-functions q)
    (if (and next (elpaca-q<-elpacas next))
        (run-hook-with-args 'elpaca-before-queue-functions next)
...
(add-hook 'elpaca-before-queue-functions #'elpaca--process-queue)
