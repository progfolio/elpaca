...
(defvar elpaca--event-subscribers (make-hash-table :test #'eq)
  "Hash table mapping event type symbols to lists of subscriber functions.
Each function must accept a single event argument")

(defun elpaca-subscribe (type fn)
  "Register FN as a subscriber to events of TYPE.
FN is a unary function which accepts an event."
  (push fn (gethash type elpaca--event-subscribers)))

(defun elpaca-unsubscribe (type fn)
  "Unregister FN from events of TYPE."
  (puthash type (cl-remove fn (gethash type elpaca--event-subscribers))
           elpaca--event-subscribers))

(defun elpaca-publish (e type &optional info verbosity &rest payload)
  "Publish an event of TYPE with associated E.
E may be nil for system-level events.
TYPE is the event type symbol; it also becomes E's new status unless E is nil.
INFO is an optional log message string recorded at VERBOSITY level.
PAYLOAD is an additional plist passed through to subscribers unchanged."
  (let* ((id (and e (elpaca<-id e)))
    ...
    (when id
      ...
      (let ((old-status (elpaca<-status e)))
        (when (not (eq type old-status))
          (unless (eq type 'package-info) (setf (elpaca<-status e) type))
            ...
            (dolist (fn subs)
              (condition-case err
                ...
                ((error) (lwarn '(elpaca event) :error
                                "Subscriber %S error for elpaca-status-changed: %S"
                                fn err))))))))
    (when (and info (<= (or verbosity 0) elpaca-verbosity))
      (let ((info-event (elpaca-event<-create :type 'package-info :id id
                                              :time (current-time)
                                              :payload (list :info info
                                                             :verbosity verbosity))))
        (push info-event elpaca--event-log)
        (when id (push info-event (gethash id elpaca--event-index)))))
    (when-let* ((subscribers (gethash type elpaca--event-subscribers)))
      (dolist (fn subscribers)
        (condition-case err
            (funcall fn event)
          ((error) (lwarn '(elpaca event) :error
...
(defun elpaca-wait-for (e type value)
  ...
  (let ((key (cons type value)))
    ...
    (elpaca-publish e 'blocked (format "Waiting on condition (%S . %S)" type value)
                    0 :face 'elpaca-blocked)))
...
(defun elpaca-satisfy (type value)
  ...
  (let ((key (cons type value)))
    (when-let* ((waiting (gethash key elpaca--conditions)))
      ...
      (dolist (blocked waiting)
        (setf (elpaca<-conditions blocked)
              (cl-remove key (elpaca<-conditions blocked) :test #'equal))
        (unless (elpaca<-conditions blocked) (elpaca-publish blocked 'unblocked))))))
...
(defun elpaca-satisfy-failed (type value reason)
  ...
  (let ((key (cons type value)))
    (when-let* ((waiting (gethash key elpaca--conditions)))
      (remhash key elpaca--conditions)
      (dolist (blocked waiting)
        ;; Clean up ALL remaining conditions, not just the failed one.
        ;; Otherwise a package waiting on multiple deps where one fails would
        ;; remain partially blocked rather than transitioning cleanly to failed.
        (dolist (other-key (elpaca<-conditions blocked))
          (unless (equal other-key key)
            (setf (gethash other-key elpaca--conditions)
                  (delq blocked (gethash other-key elpaca--conditions)))))
        (setf (elpaca<-conditions blocked) nil)
        (elpaca-publish blocked 'failed
                        (format "Condition (%S . %S) failed: %s" type value reason))))))
...
(defun elpaca--fail (e &optional reason)
  ...
  (when-let* ((p (elpaca<-process e)))
    (delete-process p))
  (elpaca-publish e 'failed reason)
...
(defun elpaca--continue-build (e &optional type info verbosity)
  ...
  (when type (elpaca-publish e type info verbosity))
  (unless (or (memq (elpaca--status e) elpaca--inactive-states)
              (and (elpaca<-process e) (process-live-p (elpaca<-process e))))
    (if-let* ((elpaca-queue-limit)
              ((not (elpaca<-builtp e)))
              ;; Only count active packages.
              ;; Queued, blocked packages are not consuming resources.
              (active (cl-loop for (_ . e) in (elpaca-q<-elpacas (elpaca--q e))
                               count (not (memq (elpaca--status e)
                                                (cons 'queued elpaca--inactive-states)))))
        ...
        (elpaca-wait-for e 'package-throttled (elpaca-q<-id (elpaca--q e)))
      (when-let* ((last (elpaca<-last-step e)))
        (elpaca-publish e 'step-completed nil 1 :step last
                        :info (format "step %s completed" last) :face 'elpaca-finished))
      (let ((step (or (pop (elpaca<-build-steps e)) #'elpaca--finalize)))
        (setf (elpaca<-last-step e) step)
        (elpaca-publish e 'step-started nil 1 :step step
                        :info (format "step %s started" step)
        ...
        (condition-case err
            (if-let* ((vars (plist-get (elpaca<-recipe e) :vars))
              ...
              (funcall step e))
          (elpaca-build-error
           (elpaca-publish e 'step-failed nil 1 :step step :error err)
...
(defun elpaca--finalize-queue (q)
  ...
  (let ((next (nth (1+ (elpaca-q<-id q)) (reverse elpaca--queues))))
    (unless (or elpaca-after-init-time
      ...
      (elpaca-publish nil 'init-completed nil nil :time (current-time)))
    (if (and next (elpaca-q<-elpacas next))
        (elpaca-publish nil 'queue-started nil nil :queue next)
      (unless elpaca--waiting (run-hooks 'elpaca--post-queues-hook)))))

(defun elpaca--finalize (e)
  "Declare E finished."
  (let ((status (elpaca--status e)))
    (elpaca-publish e (if (eq status 'failed) 'failed 'finished)
                    (concat (unless (eq status 'finished) "x " "✓ ")
                            (format-time-string "%s.%3N" (elpaca--log-duration e)) " secs")
                    nil)))
    ...
    (unless (memq (elpaca--status e) elpaca--inactive-states)
      (process-put process :timer
                   (run-at-time elpaca-busy-interval nil
                                (lambda () (unless (memq (elpaca--status e)
                                                         elpaca--inactive-states)
                                             (elpaca-publish e 'busy))))))
    (unless linep
    ...
    (dolist (line lines)
      (unless (string-empty-p line)
        (let* ((escaped (replace-regexp-in-string "\033\\[[0-9;]*[a-zA-Z]" "" line))
               ...
               (id (elpaca<-id e)))
          ;; Write directly to event index to bypass elpaca-publish overhead.
          (push (elpaca-event<-create :type 'process-output :id id
                                      :time (current-time)
                                      :payload (list :output text))
                (gethash id elpaca--event-index))
          ;; Dispatch to existing registered subscribers (e.g. UI extensions).
          (when-let* ((subs (gethash 'process-output elpaca--event-subscribers)))
            (let ((event (car (gethash id elpaca--event-index))))
              (dolist (fn subs)
                (condition-case err
                    (funcall fn event)
                  ((error) (lwarn '(elpaca event) :error
...
(defun elpaca--process-sentinel (&optional info status process event)
  ...
  (when-let* (((memq (process-status process) '(exit signal)))
              ...
              ((not (memq (elpaca--status e) elpaca--inactive-states))))
    (if (= (process-exit-status process) 0)
        (progn
          (elpaca--propertize-subprocess process)
          (elpaca-publish e 'process-finished info)
          (elpaca--continue-build e status info))
      (elpaca-publish e 'process-failed nil nil :event event)
...
(defun elpaca-build-docs-process-sentinel (process event)
  ...
  (let* ((e  (process-get process :elpaca))
    ...
    (unless (memq (elpaca--status e) elpaca--inactive-states)
      ...
      (elpaca--propertize-subprocess process)
      (if finished
          (progn (elpaca-publish e 'process-finished "Info compiled")
...
(defun elpaca--make-process (e &rest spec)
  ...
  (let* ((command (plist-get spec :command))
         (_ (elpaca-publish e 'package-info
                            (propertize (elpaca--command-string command) 'face 'elpaca-blocked)))
    ...
    (elpaca-publish e 'process-started nil nil :command command)
...
(defun elpaca-build-docs (e)
  ...
  (elpaca-publish e 'package-info "Compiling Info files")
...
(defun elpaca--install-info-async (file dir e)
  ...
  (elpaca-publish e 'package-info file)
...
(defun elpaca--install-info (e)
  ...
  (when-let* ((elpaca-install-info-executable)
    ...
    (elpaca-publish e 'package-info "Installing Info files")
...
(defun elpaca-queue-dependencies (e)
  ...
  (let ((dependencies (cl-loop for (id . _) in (elpaca--dependencies e)
    ...
    (if (null dependencies)
        (elpaca--continue-build e 'package-info "No external dependencies")
      (let* ((q (elpaca--q e))
        ...
        (dolist (dep-id dependencies)
          (let* ((queued-dep (elpaca-alist-get dep-id enqueued))
            ...
            (if (eq d-status 'finished)
              ...
              (elpaca-wait-for e 'finished dep-id))))
        (if (= finished (length dependencies))
            (elpaca--continue-build e nil "Dependencies finished")
...
(defun elpaca-build-autoloads (e)
  ...
  (if-let* ((recipe (elpaca<-recipe e))
      ...
      (progn
        (elpaca-publish e 'autoloads (concat "Generating autoloads: " default-directory))
        (elpaca-with-emacs e
...
Loads or caches autoloads."
  (let ((default-directory (elpaca<-build-dir e)))
    ...
    (elpaca-publish e 'package-info "Package build dir added to load-path")
    (if-let* ((recipe (elpaca<-recipe e))
        ...
        (if elpaca-cache-autoloads
            (let ((forms nil))
              (elpaca-publish e 'package-info "Caching autoloads")
              ...
              (elpaca-publish e 'package-info "Autoloads cached"))
          (condition-case err
              (progn
                (let ((load-source-file-function nil)) (load autoloads nil 'nomessage))
                (elpaca-publish e 'activated "Package activated"))
            ((error) (elpaca-publish e 'failed-to-activate
                                     (format "Failed to load %S: %S" autoloads err)))))
      (and (stringp key)
           (elpaca-publish e 'package-info
                           (concat "nonexistent :autoloads file \"" key "\""))))
    (elpaca--continue-build e)))
...
With a prefix argument, rebuild current file's package or prompt if none found."
  ...
  (let ((e (or (elpaca-get id) (user-error "Package %S is not queued" id))))
    ...
    (elpaca--unprocess e)
    (elpaca-publish e 'queued "Rebuilding")
...
If INTERACTIVE is non-nil, process queues."
  ...
  (cl-loop with (seen repos)
           ...
           (let* ((repo (elpaca<-source-dir e))
             ...
             (if (not mono-repo)
                 (elpaca-publish e (if (elpaca<-conditions e) 'blocked 'queued) "Updating")
               (elpaca-wait-for e 'finished mono-repo)
               (when (cdr deps) (elpaca-publish e 'blocked nil nil :mono-repo mono-repo))
               (elpaca-publish e 'blocked (format "Waiting for mono-repo %s" mono-repo)))
             (push id seen)
               ...
               `((cl-loop
                  ...
                  do (push `(lambda (e)
                              (elpaca-with-dir e ,',(or (plist-get args :dir) 'source)
                                (elpaca-publish e 'script ,stepname)
                                ;; 1. Block the package so elpaca--continue-build stops here
                                (elpaca-wait-for e 'process ,stepname)
                                (elpaca--make-process e
                                  :name ,stepname
                                  :buffer ,stepname
                                  ...
                                  :sentinel
                                  (lambda (process event)
                                    ;; 2. Satisfy the condition to allow the next step
                                    (elpaca-satisfy 'process ,stepname)
                                    (elpaca--process-sentinel nil nil process event)))))
                           (elpaca<-build-steps e))
                  finally (elpaca--continue-build e)))
             `((elpaca-with-dir e ,(or (plist-get args :dir) 'source)
                 (elpaca-publish e 'script ,(format "%s" name))
...
(defun elpaca--maybe-log-on-failure (_event)
  "Log if EVENT is a failure during init."
  (when (not elpaca-after-init-time) (elpaca--maybe-log)))

(defun elpaca--satisfy-finished (event)
  "Satisfy finished condition for EVENT's package."
  (elpaca-satisfy 'finished (elpaca-event<-id event)))

(defun elpaca--satisfy-failed (event)
  "Fail finished condition for EVENT's package."
  (let ((id (elpaca-event<-id event)))
    (when (gethash (cons 'finished id) elpaca--conditions)
      (elpaca-satisfy-failed 'finished id (format "%S failed" id)))))

(defun elpaca--continue-if-unblocked (event)
  "Continue building E from EVENT if all conditions are satisfied."
  (when-let* ((e (elpaca-get (elpaca-event<-id event)))
              ((null (elpaca<-conditions e))))
    (elpaca--continue-build e nil 'unblocked)))

(defun elpaca--release-throttled (event)
  "Release throttled packages for EVENT's queue up to `elpaca-queue-limit'."
  (when-let* ((e (elpaca-get (elpaca-event<-id event)))
              (q (elpaca--q e))
              (q-id (elpaca-q<-id q))
      ...
      (cl-loop repeat (max 0 (- (or elpaca-queue-limit 0) active))
               ...
               do (let ((waiter (car waiters))
                    ...
                    (unless (elpaca<-conditions waiter)
                      (elpaca-publish waiter 'unblocked)))))))

(defun elpaca--process-next-queue (event)
  "Process the queue from EVENT's queue-started payload."
  (elpaca--process-queue (plist-get (elpaca-event<-payload event) :queue)))

(defun elpaca--finalize-completed-queue (event)
  "Finalize the queue from EVENT's queue-completed payload."
  (elpaca--finalize-queue (plist-get (elpaca-event<-payload event) :queue)))

(defun elpaca--run-after-init-hook (_event)
  "Run `elpaca-after-init-hook'."
...
(defun elpaca--subscribe ()
  ...
  (elpaca-subscribe 'finished  #'elpaca--satisfy-finished)
  (elpaca-subscribe 'failed    #'elpaca--satisfy-failed)
  (elpaca-subscribe 'unblocked #'elpaca--continue-if-unblocked)
  ...
  (elpaca-subscribe 'queue-started   #'elpaca--process-next-queue)
  (elpaca-subscribe 'queue-completed #'elpaca--finalize-completed-queue)
  (elpaca-subscribe 'init-completed  #'elpaca--run-after-init-hook)
  (elpaca-subscribe 'process-output  #'elpaca--log-update-subscriber)
...
(defun elpaca--check-queue-completion (event)
  ...
  (when-let* ((e (elpaca-get (elpaca-event<-id event)))
    ...
    (elpaca-publish nil 'queue-completed nil nil :queue q)))

(defun elpaca--log-update-subscriber (event)
  "Update log buffer debounced after EVENT."
  (when-let* ((verbosity (or (plist-get (elpaca-event<-payload event) :verbosity) 0))
              ((<= (or verbosity 0) elpaca-verbosity)))
