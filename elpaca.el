...
(defcustom elpaca-default-build-steps '(elpaca-get
                                        elpaca--enqueue-dependencies
...
(cl-defgeneric elpaca-get (e)
  "Populate E's source directory with E's files."
  (error "No elpaca-get method for :type %S" (plist-get (elpaca<-recipe e) :type)))
...
(cl-defgeneric elpaca-source-dir (e)
  ...
  (expand-file-name (elpaca<-package e) elpaca-repos-directory))
...
(defun elpaca--enqueue-dependencies (e)
...
(defun elpaca--expand-declaration (order body)
  ...
  (let* ((id (elpaca--first order))
         (q (or (and after-init-time (elpaca--q (elpaca-find id))) (car elpaca--queues)))
           ...
           (if-let* ((menus (cl-remove 'elpaca-menu-declarations elpaca-menu-functions))
                     (elpaca-menu-functions (list (lambda (&rest _)
                                                    (cl-loop for menu in menus append
                                                             (cl-remove-if #'elpaca-find (funcall menu 'index) :key #'car)))))
                     (item (elpaca-menu-item)))
               (cons (car item) (plist-get (cdr item) :recipe))
...
(cl-defgeneric elpaca--delete (e)
  ...
  (let* ((source (elpaca<-source-dir e))
    ...
    (if (cl-some #'elpaca--on-disk-p dependents)
      ...
      (when (member source (list user-emacs-directory elpaca-builds-directory elpaca-repos-directory))
...
do not confirm before deleting package and DEPS."
  ...
  (if-let* ((e (or (elpaca-find id)
                   (let ((dir (expand-file-name (concat (symbol-name id) "/") elpaca-repos-directory)))
...
With a prefix argument, rebuild current file's package or prompt if none found."
  (interactive (list (or (and-let* ((current-prefix-arg)
                     ...
                     t))
  (let ((e (or (elpaca-find id) (user-error "Package %S is not queued" id))))
  ...
  (cl-loop with (seen repos)
           ...
           (let* ((repo (elpaca<-source-dir e))
             ...
             (if (not mono-repo)
               ...
               (cl-pushnew id (elpaca<-blocking (elpaca-find mono-repo)))
...
(defun elpaca-browse (id)
  ...
  (if-let* ((found (or (elpaca-find id)
