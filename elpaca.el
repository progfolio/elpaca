  ...
  (let* ((id (elpaca--first declaration))
    ...
    (condition-case err
        (progn
          ...
          (when-let* (((elpaca<-sourcedp e))
                      ((elpaca<-builtp e))
                      ((plist-get (elpaca<-recipe e) :watch))
                      (modfile (expand-file-name ".elpaca-watch" (elpaca<-build-dir e)))
                      ((file-exists-p modfile))
                      (mods (read-from-string (with-current-buffer (get-buffer-create "  *elpaca--watcher*")
                                                (insert-file-contents modfile nil nil nil t)
                                                (buffer-string)))))
            (setf (elpaca<-builtp e) (equal (car mods)
                                            (cl-loop for (src . _) in (elpaca--files e)
                                                     for attrs = (file-attributes src)
                                                     when attrs collect (cons (file-attribute-modification-time attrs)
    ...
    (when (plist-get (elpaca<-recipe e) :watch)
      (elpaca--write-file (expand-file-name ".elpaca-watch" build-dir)
        (prin1 (mapcar (lambda (pair)
                         (when-let* ((attrs (file-attributes (car pair))))
