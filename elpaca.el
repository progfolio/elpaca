...
(defun elpaca--check-version (e)
  ...
  (cl-loop
   ...
   when (and (version-list-< (if (eq id 'emacs) (version-to-list emacs-version) version) min)
             (or (eq id 'emacs) core (not (when-let ((tag (elpaca-latest-tag dep)))
                                          (not (version-list-< (version-to-list tag) min))))))
