...
do not confirm before deleting package and DEPS."
  ...
  (let* ((e (let ((e (or (elpaca-get id)
    ...
    (when (or force (yes-or-no-p (format "Delete package %S? " id)))
      ...
      (setf (alist-get (car (cl-find (file-name-base (directory-file-name source-dir))
...
(defun elpaca-fetch-all (&optional interactive)
  ...
  (cl-loop with repos
           ...
           do (if mono-repo ; skip queued mono-repos
                ...
                (elpaca-fetch id))
           (unless mono-repo (push (cons (elpaca<-source-dir e) id) repos)))
  (when interactive (elpaca-process-queues)))
...
;;;###autoload
(defmacro elpaca-defstep (name &optional args &rest body)
  "Define a build step function and return its NAME.
The optional ARGS plist accepts the same values as `elpaca-with-emacs' ARGS.
In addition, the `:dir` specifies the package `build` or `repo`.
BODY is either one of either:
  - One or more lists of strings of the form (PROG [ARGS...]).
    Each list is executed as a command in an asynchronous subprocess.
  - One or more elisp forms to pass to `elpaca-with-emacs'."
  (declare (indent defun))
  (unless (keywordp (car-safe args))
    (push args body) (setq args nil))
  `(defun ,name (e)
     "Generated by `elpaca-defstep'."
     ,@(if (or (eq (plist-get args :type) 'elisp)
               (cl-every (lambda (el) (stringp (car-safe el))) body))
           `((cl-loop
